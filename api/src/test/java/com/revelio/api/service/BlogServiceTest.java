...
class BlogServiceTest {
  ...
  void testFilterPublishedPostsWithNoPublishedPosts() {
    List<Blog> allUnpublished = new ArrayList<>();
    Blog.Author author = new Blog.Author("John Doe", "https://example.com/john.jpg");
    ...
    List<Blog> result = blogService.filterPublishedPosts(allUnpublished);
  ...
  @Test
  void testFilterPublishedPostsWithMixedList() {
    List<Blog> mixed = new ArrayList<>();
    Blog.Author author = new Blog.Author("John Doe", "https://example.com/john.jpg");
    ...
    List<Blog> result = blogService.filterPublishedPosts(mixed);

    assertEquals(2, result.size());
    assertTrue(result.stream().allMatch(Blog::isPublished));
    assertEquals("Published Post", result.get(0).getTitle());
    assertEquals("Another Published Post", result.get(1).getTitle());
  }
  ...
  @Test
  void testSeedDataHasUniqueIdForNewBlogPost() {
    BlogService service = new BlogService();
    List<Blog> result = service.getPublishedBlogs(0, 10);

    long count = result.stream()
        .filter(blog -> blog.getId() == 6L)
        .count();
  ...
  @Test
  void testNewAIBlogPostAppearsInCorrectChronologicalOrder() {
    BlogService service = new BlogService();
    List<Blog> result = service.getPublishedBlogs(0, 10);

    assertTrue(result.size() >= 1);
    assertEquals("Development in the era of AI", result.get(0).getTitle());
  }
}
